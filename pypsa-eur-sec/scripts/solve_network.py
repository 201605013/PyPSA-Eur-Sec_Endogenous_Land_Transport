...
def add_V2G_constraint(n):
    ...
    EV_b = n.links.carrier.str.contains("land transport EV")
    EV_c = n.links.index[EV_b & n.links.p_nom_extendable]
    V2G_b = n.links.carrier.str.contains("V2G")
    V2G_c = n.links.index[V2G_b & n.links.p_nom_extendable]
    
    if EV_c.empty or ('Link', 'p_nom') not in n.variables.index:
        return

    link_p_nom = get_var(n, "Link", "p_nom") # It finds the 'number' (position) of the variable
    
    lhs = linexpr((1/(snakemake.config['sector']['EV_power']*2),link_p_nom[EV_c].values),(-1/snakemake.config['sector']['bev_charge_rate'],link_p_nom[V2G_c]))
        
    define_constraints(n, lhs, ">=", 0, 'Link', 'V2G_limit') # Unit: MW
    
def add_BEV_charger_constraint(n):
        
    EV_b = n.links.carrier.str.contains("land transport EV")
    EV_c = n.links.index[EV_b & n.links.p_nom_extendable]
    BEV_b = n.links.carrier.str.contains("BEV charger")
    BEV_c = n.links.index[BEV_b & n.links.p_nom_extendable]
    ...
    lhs = linexpr((1/snakemake.config['sector']['EV_power'],link_p_nom[EV_c].values),(-1/snakemake.config['sector']['bev_charge_rate'],link_p_nom[BEV_c]))
    
    define_constraints(n, lhs, ">=", 0, 'Link', 'BEV_limit') # Unit: MW
    
def add_EV_storage_constraint(n):
        
    EV_b = n.links.carrier.str.contains("land transport EV")
    EV_c = n.links.index[EV_b & n.links.p_nom_extendable]
    StoreEV_b = n.stores.carrier.str.contains("Li ion")
    StoreEV_c = n.stores.index[StoreEV_b & n.stores.e_nom_extendable]
    ...
    store_e_nom = get_var(n,"Store","e_nom")
    
    investment_year = int(snakemake.wildcards.planning_horizons)

    lhs = linexpr((1/snakemake.config['sector']['EV_power'],link_p_nom[EV_c].values),(-1/snakemake.config['sector']['EV_energy_cap'].get(investment_year),store_e_nom[StoreEV_c]))
    
    define_constraints(n, lhs, ">=", 0, 'Store', 'Store_limit') # Unit: MW
...
def extra_functionality(n, snapshots):
    ...
    add_V2G_constraint(n) # ADDED THIS
    add_BEV_charger_constraint(n) # ADDED THIS
    add_EV_storage_constraint(n) # ADDED THIS
...
def solve_network(n, config, opts='', **kwargs):
    ...
    if cf_solving.get('skip_iterations', False):
        network_lopf(n, solver_name=solver_name, solver_options=solver_options,keep_references=True,
                     extra_functionality=extra_functionality, 
                     keep_shadowprices=keep_shadowprices, **kwargs)
    else:
        ilopf(n, solver_name=solver_name, solver_options=solver_options,keep_references=True,
...
if __name__ == "__main__":
    if 'snakemake' not in globals():
        ...
        snakemake = mock_snakemake(
            ...
            clusters=37,
            lv=1.0,
            sector_opts='Co2L0.5-672H-T-H-B-I-solar3-dist1',
