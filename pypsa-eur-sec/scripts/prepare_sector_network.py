...
def prepare_data(n):
    ...
    nodal_transport_data["vehicle-km driven"] = pop_layout["fraction"] * nodal_transport_data["vehicle-km driven"]
    nodal_transport_data["km per vehicle"] = nodal_transport_data["vehicle-km driven"] / nodal_transport_data["number cars"] # average km per vehicle (Does not change with more nodes)
    ...
    nodal_transport_data["ICEV_power"] = V*nodal_transport_data["average fuel efficiency"] # Used to calculate FOM
    nodal_transport_data["EV_power"] = V*battery_to_wheels_eta
    nodal_transport_data["ICEV_FOM"] = ((snakemake.config["sector"]["ICEV_main"]*nodal_transport_data["km per vehicle"]/nodal_transport_data["ICEV_power"])*10**3 / get(options["ICEV_overnight_cost"], investment_year))*100
    # FOM for EV as percentage of overnight cost
    nodal_transport_data["EV_FOM"] = ((snakemake.config["sector"]["EV_main"]*nodal_transport_data["km per vehicle"]/nodal_transport_data["EV_power"])*10**3 / get(options["EV_overnight_cost"], investment_year))*100
    ...
    avail_max = options.get("bev_avail_max", 0.98)
    avail_mean = options.get("bev_avail_mean", 0.92)
...
def add_land_transport(n, costs):
    # TODO options?
    # Adding endogenous electrification of transport.
    ...
    if options["endogenous_transport"] == False:
        ...
        if electric_share > 0:
            ...
            if "Li ion" not in n.carriers.index:
    ...
    else: # Electrification of transport endogeneous
        #   ---------- EV calculations ----------
            EV_overnight=get(options["EV_overnight_cost"], investment_year) # EV car overnight cost
            
            nodes = pop_layout.index
        
        #   ---------- ICEV calculations ----------
            ICEV_overnight=get(options["ICEV_overnight_cost"], investment_year) # ICEV car overnight cost

        # Power requirement for transport (MW_elec). Added to land transport load
            p_set = (transport[nodes] + cycling_shift(transport[nodes], 1) + cycling_shift(transport[nodes], 2)) / 3
        ...
        #   ------------------------------------------------------ #
            ...
            n.madd("Load",
                ...
                bus=nodes + " Transport demand",
                carrier="land transport",
                p_set=p_set
            )


            capital_cost_ICEV = ((annuity(snakemake.config["sector"]["lifetime_ICEV"], snakemake.config["sector"]["vehicle_discountrate"]) +
                             nodal_transport_data["ICEV_FOM"]/100.) * ICEV_overnight)
            
            # capital_cost_ICEV_mean = sum(capital_cost_ICEV)/len(capital_cost_ICEV) # Take the mean capital cost for each node.

            # Add a link between EV battery bus and low voltage bus
            n.madd("Link",
                nodes,
                suffix=" BEV charger",
                bus0=nodes,
            ...
            avail_driving = 1-n.links_t.p_max_pu['AL0 0 BEV charger']
            
            # Capital cost is per year (annualized) for EV
            capital_cost_EV = ((annuity(snakemake.config["sector"]["lifetime_EV"], snakemake.config["sector"]["vehicle_discountrate"]) +
                            nodal_transport_data["EV_FOM"]/100.) * EV_overnight)
            # capital_cost_EV_mean = sum(capital_cost_EV)/len(capital_cost_EV) # Take the mean capital cost for each node.
            ...
            for ind in nodes:
                ...
                ind+" ICEV",
                bus0="EU oil",
                bus1=ind+" Transport demand",
                bus2="co2 atmosphere",
                carrier="land transport ICEV",
                p_nom_extendable=True,
                p_max_pu=avail_driving, # Number of vehicles available (i.e, vehicles driving)
                capital_cost=capital_cost_ICEV[ind],
                efficiency=ice_efficiency,
                efficiency2=costs.at["oil", 'CO2 intensity'],
                lifetime=snakemake.config["sector"]["lifetime_ICEV"]
                        )
                ...
                carrier="land transport EV",
                p_nom_extendable=True,
                p_max_pu=avail_driving,
                capital_cost=capital_cost_EV[ind],
                efficiency=1,
                lifetime=snakemake.config["sector"]["lifetime_EV"]
                    )
            ...
            if options["v2g"]:
                ...
                n.madd("Link",
                    ...
                    carrier="V2G",
                    p_nom_extendable=True,
                    marginal_cost=get(options["V2G_marginal_price"], investment_year),
                    p_max_pu=avail_profile[nodes], # Number of vehicles available (i.e, vehicles parked)
                    efficiency=options.get("bev_charge_efficiency", 0.9),
                        )
            if options["bev_dsm"]:
            ...
            capital_costs = pd.concat([capital_cost_EV,capital_cost_ICEV],axis=1)
            
            return capital_costs # Return capital cost to use in add_existing_baseyear.py
                 #       -------------------------------------       #
...
if __name__ == "__main__":
    if 'snakemake' not in globals():
        ...
        snakemake = mock_snakemake(
            ...
            clusters="37",
            lv=1.0,
            sector_opts='Co2L0-H-T-H-B-I-solar3-dist1',
    ...
    if not investment_year == 2020: 
        costs.at['coal', 'fuel'] = snakemake.config['costs']['coal_price']
        costs.at['oil', 'fuel'] = snakemake.config['costs']['oil_price']
        costs.at['lignite', 'fuel'] = snakemake.config['costs']['lignite_price']
        costs.at['gas', 'fuel'] = snakemake.config['costs']['gas_price']

    if not investment_year == 2020:     
        for o in opts:      
            if o[:3] == "gas":
                print('Gas price pr MWh multiplied by:',float(o[3:].replace("p", ".").replace("m", "-")) )
                costs.at['gas', 'fuel'] = float(o[3:].replace("p", ".").replace("m", "-"))         
                
        for o in opts:      
            if o[:3] == "oil":
                print('Oil price pr MWh is set to equivalent price pr. BBL:',float(o[3:].replace("p", ".").replace("m", "-")) )
                costs.at['oil', 'fuel'] = float(o[3:].replace("p", ".").replace("m", "-"))
    
        for o in opts:      
            if o[:4] == "coal":
                print('coal price pr MWh is set to equivalent price pr. Metric Tonne of:',float(o[4:].replace("p", ".").replace("m", "-")) )
                costs.at['coal', 'fuel'] = float(o[4:].replace("p", ".").replace("m", "-")) 
                costs.at['lignite', 'fuel'] = costs.at['coal', 'fuel']/2.81 #setting the price of ligninte according to the ratio with coal
    ...
    if "T" in opts:
        ...
        capital_costs = add_land_transport(n, costs) # Changed this to save capital_costs
    ...
    if options["endogenous_transport"] == True:
    	capital_costs.to_csv(snakemake.output.capital_costs_name)
 
    n.export_to_netcdf(snakemake.output.network_file_name)
