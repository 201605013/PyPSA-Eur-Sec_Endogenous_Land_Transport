...
if __name__ == "__main__":
    ...
    carriers = ['lignite','coal','oil','uranium','co2','co2 stored','gas','solid biomass','biogas']
    for carrier in carriers: # n.stores.carrier.unique():
    ...
    carriers = ['coal','lignite','oil','gas']     
    for carrier in carriers: #carriers: #n.generators.carrier.unique():
        ...
        if len(df['year'].unique()) > 1:
            ...
            index_remove = df[~df.index.isin(index_to_keep)].index
            df_remove = df.loc[index_remove]
            # Set p_nom, p_nom_opt and p_initial for previous years stores
            df_remove.p_nom = 0
            df_remove.p_nom_opt = 0
            df_remove.p_nom_max = 0
            #df_remove.e_initial = 0
            df.drop(index=df_remove.index ,inplace=True)
            df = df.append(df_remove) 
            n.generators[n.generators.carrier == carrier] = df
            print(carrier + ' updated')
